...
function resolveUserIdFromSessionUser(user?: SessionUser): number | undefined {
  // accept both `id` and `sub` to handle provider/session shape differences.
...
async function getUserAndGuestContext() {
  // centralized context helper for identical session/cookie resolution.
...
export async function getOrCreateCartService() {
  ...
  if (userId) {
    // authenticated path always prefers stable user cart.
  ...
  let gid = guestCartId;
  if (!gid) {
    // guest carts get a long-lived UUID cookie for anonymous persistence.
    gid = randomUUID();
...
export async function addToCartService(productId: number, quantity = 1) {
  ...
  if (existing) {
    // existing line is incremented but capped by live stock.
...
export async function updateCartItemService(
  ...
  if (!product || !product.isActive || product.stock < 1) {
    // remove stale/unavailable items instead of keeping broken cart lines.
    await deleteCartLineItem(existing.id);
    return getOrCreateCartService();
  }
...
export async function mergeGuestCartToUserService() {
  ...
  let merged = null;
  if (!userCart) {
    // fast path claims guest cart directly when user has no cart yet.
    merged = await assignGuestCartToUser(guestCartId, userId);
  } else {
    // merge path combines line quantities and resolves stock constraints.
...
export async function getCartWithLiveProductsService() {
  ...
  // expose product snapshot under `liveProduct` to keep frontend contract explicit.
  return {
    ...cart,
