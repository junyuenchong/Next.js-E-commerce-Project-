...
/* Cookies */
export async function setSessionTokenCookie(token: string, expires: Date): Promise<void> {
...
export const loginUser = async (email: string, password: string) => {
    ...
    // Fix: Check if passwordHash exists
    if (!user.passwordHash) {
        return {
            user: null,
            error: "This account does not have a password. Please sign in with Google or Facebook.",
        }
    }
    ...
    // Merge guest cart into user cart if guest cart token exists
    const cookieStore = await cookies();
...
export const logoutUser = async () => {
    ...
    // Do not clear cookies here; let the API route handle it.
}