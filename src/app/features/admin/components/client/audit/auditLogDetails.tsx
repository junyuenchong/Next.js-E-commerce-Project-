...
function toHumanLabel(value: string | undefined): string {
...
function metadataSummary(metadata: unknown): string | null {
  ...
  for (const [key, rawValue] of Object.entries(metadata)) {
    ...
    if (!value) continue;
    parts.push(`${toHumanLabel(key)}: ${value}`);
...
function formatAuditSentence(
  ...
  if (!isRecord(metadata)) {
    return action ? `${toHumanLabel(action)} completed.` : "Action completed.";
  }
  ...
  if (action && summary)
    return `${toHumanLabel(action)} completed (${summary}).`;
  if (action) return `${toHumanLabel(action)} completed.`;
  return summary ? `Action completed (${summary}).` : "Action completed.";
}
