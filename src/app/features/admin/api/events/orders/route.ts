...
// Open an SSE stream for order-related admin events.
export async function GET(request: Request) {
  try {
    ...
    const channel = ADMIN_SSE_CHANNELS.orders;
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        sendSseEvent({ type: "status", realtime: true, resource: "orders" });
    ...
    return jsonInternalServerError(
      error,
      "[admin/api/events/orders GET]",
      "Failed to open orders event stream",
    );
  }
}
