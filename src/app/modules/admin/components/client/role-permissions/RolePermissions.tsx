...
export default function RolePermissions() {
  ...
  const savePermissions = async (roleId: number) => {
    ...
    try {
      ...
      await http.patch("/modules/admin/api/role-config", {
        roleId,
        permissionIds,
      });
  ...
  const createProfile = async () => {
    ...
    setMsg(null);
    try {
      await http.post("/modules/admin/api/role-config", { slug, name });
      setNewSlug("");
      setNewName("");
  ...
  const removeProfile = async (roleId: number) => {
    ...
    setMsg(null);
    try {
      await http.delete(`/modules/admin/api/role-config?id=${roleId}`);
    ...
    <div className="space-y-8 max-w-5xl">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-600 mt-1">
            Custom profiles attach to staff/admin accounts on the Users page.
            Built-in super admin permissions are not edited here.
          </p>
        </div>
        <Link
      ...
      <div className="space-y-6">
        {roles.map((role) => {
          ...
          const canEdit = data.canEditProfiles;
