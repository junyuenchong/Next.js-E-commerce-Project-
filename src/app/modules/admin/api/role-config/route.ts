...
} from "@/app/modules/admin/schema/role-config.schema";
import {
  adminApiRequire,
  adminJsonForbidden,
...
async function canMutateRoleDefinition(
  ...
  if (def.isSystem) return false;
  return adminUserHasPermission(user, "role.profile.update");
}
...
export async function GET() {
  ...
  try {
    ...
    const canEditProfiles =
      ...
      (await adminUserHasPermission(g.user, "role.profile.update"));
    const canAddProfile = g.user.role === "SUPER_ADMIN";
    const canDeleteProfile =
      g.user.role === "SUPER_ADMIN" ||
      (await adminUserHasPermission(g.user, "role.profile.delete"));
    ...
    console.error("[admin/role-config GET]", e);
    const message = e instanceof Error ? e.message : "Database error";
    return NextResponse.json(
      {
        error: "role_config_failed",
        message,
        hint: "Ensure the database schema matches this application (see your deployment or DBA process).",
      },
      { status: 500 },
    );
  }
...
export async function PATCH(request: Request) {
  ...
  if (metaParsed.success) {
    ...
    const def = roleHasIsActive
      ? await prisma.adminRoleDefinition.findUnique({ where: { id: roleId } })
      : await prisma.adminRoleDefinition.findUnique({
          where: { id: roleId },
          omit: { isActive: true },
        });
    if (!def) {
    ...
    if (!(await canMutateRoleDefinition(g.user, def))) {
      return adminJsonForbidden(
        "You need super admin or role.profile.update to edit custom permission profiles.",
      );
    }
...
export async function POST(request: Request) {
  ...
  const reserved = new Set(["super_admin", "admin", "staff"]);
  ...
  try {
    ...
    console.error("[admin/role-config POST legacy insert]", e);
    return NextResponse.json(
      {
        error: "create_failed",
        message: e instanceof Error ? e.message : "Could not create profile",
        hint: "Ensure the database schema matches this application (see your deployment or DBA process).",
      },
      { status: 500 },
    );
...
export async function DELETE(request: Request) {
  ...
  if (
    ...
    return adminJsonForbidden(
      "You need the role.profile.delete permission (or super admin) to remove custom profiles.",
    );
  }
