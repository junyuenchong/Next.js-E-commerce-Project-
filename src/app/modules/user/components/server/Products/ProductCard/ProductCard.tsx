...
import { discountPercentFromCompareAt } from "@/app/lib/format-price";
...
const ProductCard = React.memo(
  ({
    ...
    const priceN = moneyToNumber(product.price);
    const compareAtN =
      product.compareAtPrice == null
        ? null
        : moneyToNumber(product.compareAtPrice);
    const discountPct = discountPercentFromCompareAt(compareAtN, priceN);
      ...
      >
        ...
        <div className="p-3 sm:p-4 flex flex-col justify-between flex-grow gap-1 sm:gap-2">
          ...
          <div className="flex flex-wrap items-end gap-2 mt-auto">
            ...
            {compareAtN != null && compareAtN > priceN ? (
              <span className="text-sm text-gray-400 line-through">
                RM {compareAtN.toFixed(2)}
              </span>
            ) : null}
          </div>
          ...
          {user ? (
            <button
              ...
              onClick={async (e) => {
                ...
                setWishPending(true);
                try {
                  await fetch("/modules/user/api/wishlist", {
                    method: "POST",
          ...
          <Link
            href={`/modules/user/product/${product.id}`}
